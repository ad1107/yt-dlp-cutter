...
if check_ext("input.mp4"):
    ...
    if a == '1':
        ...
        print("\nExporting...")
        os.system('ffmpeg.exe -v quiet -stats -i input.mp4 -c:v libx264 ' + f'"{out_path + convertname(out_name)}"' + '.mp4')
        print("\nFinished exporting.")
        flag = 1
    elif a == '2':
        out_name = input("\nType your output name, if blank, the name will be 'output.mp4': ")
        ...
        os.system(
            'ffmpeg.exe -v quiet -stats -ss ' + str(begin) + ' -t ' + str(
                end - begin) + ' -i input.mp4 -y -c:v libx264 -c copy ' + f'{"out_path + convertname(out_name)"}' + '.mp4')
        print("\nFinished Exporting.")
        flag = 1
